...
namespace PW.FailFast
{
  ...
  public static partial class Guard
  {
    ...
    // Changed from IEnumerable<T> for safety. Possible issues if used incorrectly causing multiple execution or other side-effects.
    /// <summary>
    ...
    {
      NotNull(directory, argumentName);
    ...
    {
      NotNull(file, nameof(argumentName));
      if (!file.Exists) throw new FileNotFoundException($"Guard.{nameof(MustExist)} failed for argument: {argumentName}. File not found: " + file.Value, file.Value);
    }
    ...
    {
      NotNull(o, argumentName);
      if (!o.Exists) throw new DirectoryNotFoundException("File system object not found: " + o.FullName);
    }
